      ...
      <table className="min-w-full divide-y divide-gray-700">
        <thead className="bg-gray-900/50">
          <tr>
            ...
            {softwares.map((sw) => (
              <th key={sw.id} className="px-2 py-4 text-center text-xs font-medium uppercase tracking-wider group relative min-w-[140px]">
        ...
        <tbody className="divide-y divide-gray-700">
          {dimensions.map((dim) => (
            <tr key={dim.id} className="hover:bg-gray-700/30 transition-colors group/row">
              <td className="px-4 py-4 text-sm font-medium text-gray-300 sticky left-0 bg-gray-800 z-10 align-top shadow-[2px_0_5px_-2px_rgba(0,0,0,0.5)]">
                <div className="flex items-center gap-2 mt-2">
                  <button
                    ...
                    className="text-gray-600 hover:text-red-400 transition-colors p-1 rounded opacity-60 hover:opacity-100 -ml-2"
                    title="删除该维度"
                  >
                    <Trash2 size={14} />
                  </button>
              ...
              {softwares.map((sw) => {
                ...
                return (
                  <td key={`${sw.id}-${dim.id}`} className="px-2 py-3 align-top relative">
                    <div className="flex flex-col items-center gap-3">
                      {/* Score Input */}
                      <div className="flex items-center justify-center gap-1">
                        <span className="text-[10px] text-gray-500 uppercase tracking-tighter">评分</span>
                        <input
                          type="number"
                          min="0"
                          max="10"
                      ...
                      {description ? (
                        <div className="relative group/tooltip w-full">
                          ...
                          >
                            ...
                            <Edit2 size={10} className="text-gray-500 opacity-0 group-hover:opacity-100 ml-auto shrink-0" />
                          </button>
                          ...
                          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-72 p-3 bg-gray-900 text-xs text-gray-200 rounded-lg shadow-xl border border-gray-600 hidden group-hover/tooltip:block z-50 pointer-events-none whitespace-pre-wrap break-words leading-relaxed text-left">
                            <div className="font-semibold text-blue-400 mb-1 border-b border-gray-700 pb-1">详细描述</div>
                            {description}
                            {/* Arrow */}
